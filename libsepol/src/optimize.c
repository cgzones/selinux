			       ...
			       unsigned char not_cond)
{
	 ...
	 * empty avrule)
	 */
	if (entry->key.specified & AVTAB_AV && !(entry->key.specified & AVTAB_AUDITDENY)) {
		const class_datum_t *cladatum = p->class_val_to_struct[entry->key.target_class - 1];
		uint32_t mask = PERMISSION_MASK(cladatum->permissions.nprim);

		/* inverse logic needed for AUDITDENY rules */
		if (entry->key.specified & AVTAB_AUDITDENY) {
			d1->data |= mask;
			if (d1->data == UINT32_C(0xFFFFFFFF))
				return 1;
		} else {
			d1->data &= mask;
			if (d1->data == 0)
